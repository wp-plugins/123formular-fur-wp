<?php
/*
Plugin Name: 123Formular for WordPress
...
{
    array_push($buttons, "separator", "123formular");
    return $buttons;
}

function contact_123_register($plugin_array)
{
    $url = trim(get_bloginfo('url'), "/")."/wp-content/plugins/123formular-for-wordpress/editor_plugin.js";
		...
		$pos_custom_controls=strpos($tosearch,"control",$i+19);
	        ...
	        if (is_numeric($id))
				...
				elseif ( is_callable('file_get_contents') ) {
					$linkcode=file_get_contents("http://www.123formular.de/embedded-link/".$id.".txt");   
					}
